##############################################################################################################
# SVM classifier to predict lapses in response inhibition on the Go/Nogo task using PSD estimates
#
#
# Karim Mithani
# July 2024
...
outdir = f'/d/gmi/1/karimmithani/seeg/analysis/gonogo/models/svm/analysis/psd_{fmax}Hz'
...
for idx, subj in enumerate(subjects):
    ...
    gs_svm.fit(X_train.reshape(X_train.shape[0], -1), y_train)
    print(f'Best hyperparameters for SVM: {gs_svm.best_params_}')
    
    #%% Train the model
    # model = SVC(kernel='linear', C=1.0, probability=True, tol=1e-3, random_state=42)
    model = SVC(**gs_svm.best_params_, probability=True, tol=1e-3, random_state=42)
    model.fit(X_train.reshape(X_train.shape[0], -1), y_train)
    # Save model hyperparameters
    with open(os.path.join(subj_outdir, f'{subj}_model_hyperparameters.json'), 'w') as f:
        json.dump(gs_svm.best_params_, f)
