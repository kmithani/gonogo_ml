##############################################################################################################
# Random forest classifier to predict lapses in response inhibition on the Go/Nogo task using PSD estimates
#
#
# Karim Mithani
# July 2024
...
outdir = f'/d/gmi/1/karimmithani/seeg/analysis/gonogo/models/randomforest/analysis/psd_{fmax}Hz'
...
for idx, subj in enumerate(subjects):
    ...
    gs_rf = GridSearchCV(RandomForestClassifier(), params, cv=5, n_jobs=10, verbose=1, scoring='f1')
    gs_rf.fit(X_train.reshape(X_train.shape[0], -1), y_train)
    print(f'Best hyperparameters for Random Forest: {gs_rf.best_params_}')
    
    # Save model hyperparameters
    with open(os.path.join(subj_outdir, f'{subj}_model_hyperparameters.json'), 'w') as f:
        json.dump(gs_rf.best_params_, f)
    
    #%% Train the model
    model = RandomForestClassifier(**gs_rf.best_params_, random_state=42, n_jobs=10)
    # model = RandomForestClassifier(n_estimators=100, criterion='gini', max_depth=None, random_state=42, n_jobs=10)
